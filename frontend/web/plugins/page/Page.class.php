	...
	*/
	class Page {
		...
		function fpage(){
			...
			$html[0] = "&nbsp;<b style=\"float:left; margin-top:8px; margin-right:8px; font-weight:normal; color:#666;\">共 {$this->total} {$this->config["head"]}</b>&nbsp;";
			$html[1] = "&nbsp;本页 <b>".$this->disnum()."</b> 条&nbsp;";
			$html[2] = "&nbsp;本页从 <b>{$this->start()}-{$this->end()}</b> 条&nbsp;";
			$html[3] = "&nbsp;<b>{$this->page}/{$this->pageNum}</b>页&nbsp;";
			$html[4] = $this->firstprev();  //控制上一页和首页
			$html[5] = $this->pageList(); //控制数字分页几页到几页
			$html[6] = $this->nextlast();   //控制下一页和未页
			$html[7] = $this->goPage();   //控制跳转页几页到几页
		...
		private function pageList(){
			...
			/*当前页的信息 */
			if($this->pageNum > 1)
				$linkPage .= "<span style='display:block;height:20px; min-width:20px; text-align:center; font-size:14px; border:1px solid #d6d6d6; float:left; margin-left:3px; padding:3px 5px; line-height:20px; text-decoration:none; background:#FF4500; border-color:#FF4500; color:#FFF; '>{$this->page}</span>";
		...
		private function goPage(){
    			if($this->pageNum > 1) {
				return '&nbsp;<input style="width:30px; height:25px; margin-left:30px; !important;height:24px;border:1px solid #CCCCCC; float:left;" type="text" onkeydown="javascript:if(event.keyCode==13){var page=(this.value>'.$this->pageNum.')?'.$this->pageNum.':this.value;location=\''.$this->uri.'page=\'+page+\'\'}" value="'.$this->page.'"><input style="cursor:pointer; width:40px; height:28px;border:1px solid #CCCCCC; float:left; margin-left:5px;" type="button" value="GO" onclick="javascript:var page=(this.previousSibling.value>'.$this->pageNum.')?'.$this->pageNum.':this.previousSibling.value;location=\''.$this->uri.'page=\'+page+\'\'">&nbsp;';
			}
		}
